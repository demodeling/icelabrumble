...
A single-page HTML5 canvas fighter: pick an IceLab fighter, dodge the charge, duck the horn, fill the meter, land your special and watch the false positive explode into DNA. The bench is short for now — **Anton** rides a skateboard and throws the rhino over his hip (Skate-Jitsu Throw), **June** kicks above her own head (Sky-High Axe Kick), **Sarah** shouts mean Australian things at it until it loses health (Strewth Barrage), **Aswin**, behind very big glasses, dresses up in black tie and talks so loudly the rhino takes damage and runs (Black-Tie Boom), **Åke** puts his fingers to his temples, gathers a Claude spark between his hands and knocks the rhino out on the spot (Claude Mind Spark), **Abigail** shoots water from her fingertips that soaks the rhino and washes it back (Fingertip Flood), **Sasha**, who looks like a maroon cactus, fires three fans of spines (Spine Storm), and **Suvam** brings the Punjab desert with him, vanishes in the sand and turns up on the rhino's back with a headlock (Desert Headlock); the rest of the IceLab members page comes back later. Twelve rounds: the eight from the sister game, Swebits Rumble, plus IceLab's own four (the last one a body shop where every hit resprays the rhino, panel by panel, until it turns out to be a fawn). Works in Safari, Chrome, iPhone, iPad and desktop; installable as a home-screen app; keyboard or touch. The CREDITS button on the title screen says what IceLab is and why it is worth celebrating.
...
**Versus (beta).** *VERSUS* on the title screen: anyone can play **the rhino** — punch is the horn sweep, kick paws the ground and charges, the special is a jet stomp. Two players on one keyboard is the classic duel (player 1 on the arrows plus J/K/L, player 2 on W/A/S/D plus F/G/H). Online, two to four players type the same room code on their own devices and pick on the fighter screen, where an extra card appears for the rhino; two is the duel again, three or four is everybody against everybody, and the last one standing wins. Five arenas (in a versus room the round picker only offers these): Kiruna, the Wasteland, the Asteroid (low gravity), the Wide Field (twice as wide, the camera follows) and Compositional, where you all share one ring, your sizes always add up to one, every hit moves size from whoever takes it to everybody else, and a side below the 6 % detection limit is out. A fight is 99 seconds; if it goes the distance, whoever holds the biggest share of their own bar wins. A player who leaves the room is out. Versus results are never saved to the leaderboard.

## Co-op

**CO-OP** on the title screen: two to four players type the same room code (say `kiruna`), pick a fighter and press READY; the round starts once everybody has. A fifth player on the same code is told the room is full. The two browsers connect directly over WebRTC using [Trystero](https://github.com/dmotz/trystero) (bundled in `src/vendor/trystero.js`, loaded only when you use co-op; signalling goes over public Nostr relays, so there is nothing to host). If direct peer-to-peer traffic is blocked (eduroam and many office networks do that), the game falls back to a relay through the project's Supabase Realtime channel, which is a plain WebSocket. The room has one host (the first two pages pair up and the lower id hosts; later arrivals join that room, and two rooms that formed on one code at the same moment merge): it runs the fight for everybody and streams snapshots 30 times a second (20 over the relay), the guests send inputs. The rhino goes for whoever is closest; the round is lost only when everybody is down. The host picks the round and starts rematches; if the host leaves, the next player in the room takes over.
