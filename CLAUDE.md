...
- Co-op: `Net` + `coopJoin()` / `netData()` (host = lower peer id). The host runs `hostTick()` → `update()` with `G.players` = [host, guest] (guest inputs arrive as `{t:'in'}`), and `sendSnap()` every 2nd frame; the guest runs `guestTick()` and `applySnap()`. Sounds are relayed through `Net.sfxQ`. Trystero is imported from `TRYSTERO_URL` on demand; tests inject `window.__coopTransport` instead. Anything that hits or targets a player must loop over `G.players` (see `rhinoTarget()`, `hitPlayer(p, …)`).
